#!/usr/bin/env python3
"""
Résolution Flexcube accorgl (A/G) via Oracle — mode thin (sans Instant Client).

Entrée JSON (stdin) :
{
  "host": "...", "port": 1522, "service_name": "...",
  "username": "...", "password": "...", "schema": "CFSFCUBS145",
  "accounts": ["379200000180", "..."]
}

Sortie JSON (stdout) :
{ "ok": true, "map": { "379200000180": "G", ... } }
"""
...
def main() -> int:
    ...
    # Mode thin : pas besoin d'Instant Client / oci8
    ...
    try:
        with oracledb.connect(user=user, password=password, dsn=dsn) as conn:
            with conn.cursor() as cur:
                ...
                result: dict[str, str] = {}
                for cpt, accorgl in cur:
                    key = str(cpt).strip() if cpt is not None else ""
                    typ = str(accorgl).strip().upper() if accorgl is not None else ""
                    if not key or typ not in ("A", "G"):
                        continue
                    # Priorité G si présent dans les deux tables
                    if key not in result or typ == "G":
                        result[key] = typ
    except Exception as exc:  # noqa: BLE001 — renvoyer le message à PHP
        print(json.dumps({"ok": False, "error": str(exc)}))
        return 1

    print(json.dumps({"ok": True, "map": result}))
    return 0
