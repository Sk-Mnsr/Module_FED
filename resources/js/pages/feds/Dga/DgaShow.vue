...
const statusBadge = (status: string) => {
    const badges: Record<string, string> = {
        daf_approved: 'bg-indigo-100 text-indigo-800 border border-indigo-200',
...
const approve = () => {
    if (!confirm('Confirmer la validation DGA et le passage en bon de commande ?')) return;
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex min-h-0 flex-1 flex-col gap-4 p-4 lg:p-6">
            <div class="flex flex-wrap items-center justify-between gap-3 print:hidden">
                <div class="flex flex-wrap items-center gap-3">
                    <Button as-child variant="outline" size="sm">
                        <Link href="/feds/dga" class="inline-flex items-center gap-1.5">
            ...
            <div class="grid min-h-0 flex-1 gap-4 lg:grid-cols-[minmax(0,1.45fr)_minmax(300px,0.7fr)]">
                <!-- Colonne gauche : fiche + tableau comparatif détaché -->
                <div class="order-2 flex min-h-0 flex-col gap-4 overflow-y-auto lg:order-1">
                    <!-- Fiche FED (document officiel) -->
                    ...
                    <div class="flex flex-col gap-4 rounded-xl border border-border bg-card p-4 shadow-sm lg:p-5">
                        ...
                        <div class="space-y-3 border-t border-border pt-4">
                            ...
                            <div>
                                ...
                                />
                                <p v-if="props.fed.dga_action_at" class="mt-1 text-xs text-muted-foreground">
                                    Traité le
                                    {{ new Date(props.fed.dga_action_at).toLocaleString('fr-FR') }}
                                </p>
                            </div>

                            <div v-if="canValidate" class="flex flex-col gap-2">
                                <Button type="button" class="w-full" @click="approve">
                            ...
                            >
                                Cette FED n’est plus en attente de validation DGA.
                            </p>
                        </div>
        ...
        <!-- Modale tableau comparatif (détaché de la fiche) -->
