...
const form = useForm({
    type: props.typology.type,
    libelle: props.typology.libelle,
    description: props.typology.description ?? '',
});

const submit = () => {
    form.put(`/typologies/${props.typology.id}`, { preserveScroll: true });
};
</script>

<template>
    <Head title="Modifier une typologie" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-6 p-6">
            <h1 class="text-3xl font-bold text-gray-900">Modifier une typologie</h1>

            <form @submit.prevent="submit" class="flex flex-col gap-6">
                <FormSection :columns="2">
                    <div>
                        <Label for="type" class="text-base font-medium text-gray-700">Type</Label>
                        <Input id="type" v-model="form.type" type="text" required class="mt-1.5 border-gray-300" />
